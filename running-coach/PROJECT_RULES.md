...
## Custom Commands
- `ship-it "<message>"`: Runs `npm run lint && tsc --noEmit && npm run build && git add . && git commit -m "$1" && git push origin main`. (Note: Ask Gemini to generate the message for the argument if needed).
